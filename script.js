...
function renderizarTrilha() {
    ...
    curriculum.forEach((modulo, index) => {
        ...
        const isNew        = estado.novoDesbloqueado === modulo.id;

        // Conector entre nós
        if (index > 0) {
            const conector = document.createElement('div');
            conector.className = `trilha-connector ${isCompleted ? 'done' : ''}`;
            ui.trilhaMap.appendChild(conector);
        }
        ...
        node.className = `trilha-node
            ...
            ${isLocked ? 'locked' : ''}
            ${isNew ? 'novo-desbloqueado' : ''}`.trim();
        node.setAttribute('role', 'listitem');

        const iconClass = isCompleted ? 'fa-check' : (isLocked ? 'fa-lock' : 'fa-book');
        const badgeHTML = isCompleted
            ? `<span class="node-badge badge-done">Concluído</span>`
            : (!isLocked ? `<span class="node-badge badge-open">Aberto</span>` : '');
        ...
        if (!isLocked) {
            ...
            node.setAttribute('aria-label', `Módulo: ${modulo.title}${isCompleted ? ' (concluído)' : ''}`);
            node.addEventListener('click', () => carregarModulo(index));
            node.addEventListener('keypress', (e) => {
                if (e.key === 'Enter' || e.key === ' ') carregarModulo(index);
            });
        }

        ui.trilhaMap.appendChild(node);
    });

    // Limpar flag de novo desbloqueado após renderizar
